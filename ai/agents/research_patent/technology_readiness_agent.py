"""Technology Readiness Level (TRL) Assessment Agent."""
...
def get_technology_readiness_agent(
    ...
    ai_logger.info(f"Initializing TechnologyReadinessAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "TRL Assessor & Commercialization Specialist"
    goal = "Evaluate technology readiness level from TRL 1 (basic principles) to TRL 9 (proven system)."
    backstory = "Deep-tech venture evaluator assessing hardware and software technology readiness levels."
