"""Research Paper Analyzer Agent for Academic & Scholarly Literature Analysis."""
...
def get_research_paper_analyzer_agent(
    ...
    """Returns a configured CrewAI Agent instance for Academic Research Paper Analysis."""
    ai_logger.info(f"Initializing ResearchPaperAnalyzerAgent: tier={model_tier}, mock={mock_mode}")

    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Academic Research & Literature Synthesizer"
    goal = "Query OpenAlex and Semantic Scholar APIs to analyze academic papers and scientific breakthroughs."
    backstory = "Principal research scientist analyzing cutting-edge academic publications and citation graphs."
    ...
    return {
        "agent_name": "research_paper_analyzer",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
