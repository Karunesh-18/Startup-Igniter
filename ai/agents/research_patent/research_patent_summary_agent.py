...
def get_research_patent_summary_agent(
    ...
    ai_logger.info(f"Initializing ResearchPatentSummaryAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Research & Patent Synthesis Director"
    goal = "Synthesize prior art, academic papers, and IP strategy into a unified patent analysis report."
    backstory = "Head of R&D intelligence consolidating academic literature, USPTO searches, and TRL readiness."
