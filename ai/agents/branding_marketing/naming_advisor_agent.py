"""Naming Advisor Agent for Startup Naming & Brand Positioning."""
...
def get_naming_advisor_agent(
    ...
    ai_logger.info(f"Initializing NamingAdvisorAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Brand Naming Specialist & Linguistic Analyst"
    goal = "Generate catchy startup name candidates, check domain availability, and evaluate brand memorability."
    backstory = "Nomenclaturist who has named 100+ tech startups and validated global trademark availability."
    ...
    return {
        "agent_name": "naming_advisor_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
