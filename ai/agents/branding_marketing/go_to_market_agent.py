...
def get_go_to_market_agent(
    ...
    ai_logger.info(f"Initializing GoToMarketAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Go-To-Market (GTM) Launch Strategist"
    goal = "Plan ProductHunt launches, HackerNews campaigns, cold outbound sales sequences, and community seeding."
    backstory = "GTM strategist orchestrating viral product launches and B2B outbound lead generation campaigns."
