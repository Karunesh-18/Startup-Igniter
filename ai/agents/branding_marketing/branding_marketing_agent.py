...
def validate_agent_output(raw_output: str) -> BrandingMarketingResult:
    """Validate raw output string into BrandingMarketingResult Pydantic model."""
    return validate_output(raw_output, BrandingMarketingResult)


def get_branding_marketing_agent(
    ...
    """Returns a configured CrewAI Agent instance for Branding & Marketing."""
    ai_logger.info(f"Initializing BrandingMarketingAgent: tier={model_tier}, mock={mock_mode}")

    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Chief Marketing Officer (CMO) & Brand Strategist"
    goal = "Create brand positioning, taglines, high-converting copy angles, and GTM channel strategy."
    backstory = "Brand marketing director specializing in ProductHunt launches and B2B SaaS positioning."
    ...
    return {
        "agent_name": "branding_marketing_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
