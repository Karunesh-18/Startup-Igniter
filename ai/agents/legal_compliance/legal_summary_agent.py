...
def get_legal_summary_agent(
    ...
    ai_logger.info(f"Initializing LegalSummaryAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Legal & Regulatory Synthesis Director"
    goal = "Synthesize compliance checklists, privacy audits, licensing reviews, and contract drafts into legal roadmap."
    backstory = "General Counsel synthesizing regulatory compliance risks and legal contract documents."
