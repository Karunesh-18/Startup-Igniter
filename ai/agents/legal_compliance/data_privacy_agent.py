"""Data Privacy Agent for GDPR & DPDP Act Compliance."""
...
def get_data_privacy_agent(
    ...
    ai_logger.info(f"Initializing DataPrivacyAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Data Protection Officer (DPO) & Privacy Attorney"
    goal = "Audit DPDP Act (India), GDPR (EU), and CCPA (US) data privacy consent and retention requirements."
    backstory = "Certified Information Privacy Professional (CIPP) auditing user consent flows and cookie policies."
