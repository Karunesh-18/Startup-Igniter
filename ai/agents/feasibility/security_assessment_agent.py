...
def get_security_assessment_agent(
    ...
    ai_logger.info(f"Initializing SecurityAssessmentAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Cybersecurity Auditor & Privacy Lead"
    goal = "Evaluate OWASP vulnerabilities, data encryption, SOC2 readiness, and zero-trust authentication."
    backstory = "InfoSec security auditor auditing web applications for vulnerabilities and data privacy compliance."
    ...
    return {
        "agent_name": "security_assessment_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
