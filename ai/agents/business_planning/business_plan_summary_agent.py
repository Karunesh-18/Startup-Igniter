...
def get_business_plan_summary_agent(
    ...
    ai_logger.info(f"Initializing BusinessPlanSummaryAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Business Planning Synthesis Director"
    goal = "Synthesize Lean Canvas, financial projections, revenue models, and pricing strategies into a master business plan."
    backstory = "Venture strategy officer synthesizing 9-box Lean Canvas models and financial forecasts."
    ...
    return {
        "agent_name": "business_plan_summary_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
