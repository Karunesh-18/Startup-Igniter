...
def get_cost_structure_agent(
    ...
    ai_logger.info(f"Initializing CostStructureAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Startup Cost Accountant & COGS Analyst"
    goal = "Calculate fixed operating expenses, variable COGS, payroll overhead, and cloud API infrastructure costs."
    backstory = "CFO advisor calculating burn rate, fixed overhead, and cost-of-goods-sold breakdown."
    ...
    return {
        "agent_name": "cost_structure_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
