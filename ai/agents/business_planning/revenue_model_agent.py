...
def get_revenue_model_agent(
    ...
    ai_logger.info(f"Initializing RevenueModelAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Revenue Streams & Monetization Strategist"
    goal = "Design recurring subscription models, usage-based billing, enterprise licensing, and transaction fee structures."
    backstory = "Monetization consultant helping SaaS and marketplace startups structure recurring revenue streams."
    ...
    return {
        "agent_name": "revenue_model_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
