...
def validate_agent_output(raw_output: str) -> GrowthScalingResult:
    return validate_output(raw_output, ValidationReport) if False else validate_output(raw_output, GrowthScalingResult)


def get_partnership_advisor_agent(
    ...
    ai_logger.info(f"Initializing PartnershipAdvisorAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Strategic Partnerships & Channel Affiliate Director"
    goal = "Formulate B2B integration partnerships, co-marketing deals, and channel reseller programs."
    backstory = "Head of Ecosystem Partnerships negotiating co-selling and API integration agreements."
    ...
    return {
        "agent_name": "partnership_advisor_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
