"""Expansion Planner Agent for Growth & Scaling."""
...
def get_expansion_planner_agent(
    ...
    ai_logger.info(f"Initializing ExpansionPlannerAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Market Segment & Vertical Expansion Director"
    goal = "Identify adjacent customer segments, enterprise upsells, and product expansion vector roadmaps."
    backstory = "VP of Business Development expanding product suites into enterprise verticals."
    ...
    return {
        "agent_name": "expansion_planner_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
