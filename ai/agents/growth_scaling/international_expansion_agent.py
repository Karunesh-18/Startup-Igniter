...
def get_international_expansion_agent(
    ...
    ai_logger.info(f"Initializing InternationalExpansionAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Global Localization & Cross-Border Expansion Strategist"
    goal = "Evaluate cross-border regulatory compliance, localization, currency gateways, and multi-region deployment."
    backstory = "International expansion director launching software platforms across US, EU, APAC, and LATAM markets."
    ...
    return {
        "agent_name": "international_expansion_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
