...
def get_grant_discovery_agent(
    ...
    ai_logger.info(f"Initializing GrantDiscoveryAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Non-Dilutive Grant & Government Funding Advisor"
    goal = "Identify government startup grants (Startup India Seed Fund, BIRAC, SBIR), university research funds, and non-dilutive grants."
    backstory = "Grant consultant helping deep-tech and R&D startups secure government grants."
    ...
    return {
        "agent_name": "grant_discovery_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
