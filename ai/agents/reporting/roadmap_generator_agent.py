...
def get_roadmap_generator_agent(
    ...
    ai_logger.info(f"Initializing RoadmapGeneratorAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Strategic Execution Roadmap Architect"
    goal = "Synthesize 12-month startup milestone execution roadmap from idea validation to series-A."
    backstory = "Chief Operating Officer mapping 12-month cross-functional execution roadmaps."
    ...
    return {
        "agent_name": "roadmap_generator_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
