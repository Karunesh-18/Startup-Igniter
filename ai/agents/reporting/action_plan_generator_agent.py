...
def get_action_plan_generator_agent(
    ...
    ai_logger.info(f"Initializing ActionPlanGeneratorAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Immediate 30-60-90 Day Action Plan Strategist"
    goal = "Formulate high-priority 30-60-90 day action items for founding team execution."
    backstory = "Startup turnaround consultant establishing 30-60-90 day milestone execution sprints."
    ...
    return {
        "agent_name": "action_plan_generator_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
