...
def get_startup_ecosystem_agent(
    ...
    ai_logger.info(f"Initializing StartupEcosystemAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Regional Startup Hub & Ecosystem Network Manager"
    goal = "Identify regional founder meetups, hackathons, co-working incubators, and startup events."
    backstory = "Ecosystem manager connecting regional startup communities and co-working hubs."
    ...
    return {
        "agent_name": "startup_ecosystem_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
