...
def get_accelerator_recommendation_agent(
    ...
    ai_logger.info(f"Initializing AcceleratorRecommendationAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Accelerator & Incubator Program Scout"
    goal = "Evaluate YC, Techstars, Antler, Surge, and regional accelerator application fit and cohort timelines."
    backstory = "Startup accelerator Scout matching high-potential founders with top global accelerator cohorts."
    ...
    return {
        "agent_name": "accelerator_recommendation_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
