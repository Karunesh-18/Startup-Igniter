...
def get_mentor_recommendation_agent(
    ...
    ai_logger.info(f"Initializing MentorRecommendationAgent: tier={model_tier}, mock={mock_mode}")
    llm = LLMFactory.get_llm(model_tier=model_tier, temperature=temperature, api_key=api_key, mock_mode=mock_mode)

    role = "Founder Mentor Matchmaker & Advisory Director"
    goal = "Match startup domain challenges with experienced domain mentors, technical advisors, and industry veterans."
    backstory = "Accelerator mentor director connecting early-stage founders with domain expert advisors."
    ...
    return {
        "agent_name": "mentor_recommendation_agent",
        "role": role,
        "goal": goal,
        "backstory": backstory,
        "llm": llm,
