"""Integration test suite for CompetitorDiscoveryAgent running against live Groq LLM API."""
...
class TestCompetitorDiscoveryAgent(unittest.TestCase):
    """Live integration tests for CompetitorDiscoveryAgent running against real Groq LLM API."""

    def test_agent_factory_instantiation(self):
        """Test instantiation of CompetitorDiscoveryAgent factory."""
        agent = get_competitor_discovery_agent(mock_mode=False)
        self.assertIsNotNone(agent)

    def test_live_competitor_discovery_edtech(self):
        """Test live CompetitorDiscoveryAgent execution for EdTech."""
        memory_mgr = ProjectMemoryManager(use_mock_store=True)
        proj_id = "proj-test-comp-edtech-101"
        ...
        analysis = run_competitor_discovery_agent(
            context=context,
        ...
        self.assertIsNotNone(context.competitor_discovery)
        self.assertEqual(context.competitor_discovery, analysis)

        # Schema & Type Assertions
        self.assertIsInstance(analysis, CompetitorDiscovery)
        self.assertTrue(len(analysis.direct_competitors) >= 1)
        self.assertTrue(len(analysis.market_leaders) >= 1)
        self.assertTrue(len(analysis.startup_challengers) >= 1)
        self.assertTrue(len(analysis.alternative_solutions) >= 1)
        self.assertTrue(len(analysis.competition_intensity.strip()) > 0)
        self.assertGreaterEqual(analysis.major_competitor_count, 0)

        # CompetitorInfo nested validation
        comp = analysis.direct_competitors[0]
        self.assertTrue(len(comp.name.strip()) > 0)
        self.assertTrue(len(comp.category.strip()) > 0)
        self.assertTrue(len(comp.description.strip()) > 0)
        self.assertTrue(len(comp.target_market.strip()) > 0)
        self.assertTrue(len(comp.primary_offering.strip()) > 0)
        ...
        saved_memory = memory_mgr.get_memory_by_key(proj_id, "competitor_discovery")
        self.assertIsNotNone(saved_memory)
        self.assertEqual(saved_memory.value["competition_intensity"], analysis.competition_intensity)

    def test_live_competitor_discovery_healthtech(self):
        """Test live CompetitorDiscoveryAgent execution for HealthTech."""
        memory_mgr = ProjectMemoryManager(use_mock_store=True)
        proj_id = "proj-test-comp-healthtech-202"
    ...
    def test_competitor_differentiation_across_domains(self):
        """Verify different domains produce dynamically distinct competitors (AgriTech vs FinTech vs ClimateTech)."""
        memory_mgr = ProjectMemoryManager(use_mock_store=True)

        agri_val = create_sample_validation_result(
            project_id="proj-comp-agri-303",
        ...
        climate_val = create_sample_validation_result(
            project_id="proj-comp-climate-505",
        ...
        agri_context = MarketResearchContext(project_id="proj-comp-agri-303", idea_validation=agri_val)
        fin_context = MarketResearchContext(project_id="proj-comp-fin-404", idea_validation=fin_val)
        climate_context = MarketResearchContext(project_id="proj-comp-climate-505", idea_validation=climate_val)

        agri_analysis = run_competitor_discovery_agent(context=agri_context, memory_manager=memory_mgr, mock_mode=False)
        fin_analysis = run_competitor_discovery_agent(context=fin_context, memory_manager=memory_mgr, mock_mode=False)
        climate_analysis = run_competitor_discovery_agent(context=climate_context, memory_manager=memory_mgr, mock_mode=False)

        self.assertIsNotNone(agri_context.competitor_discovery)
        self.assertIsNotNone(fin_context.competitor_discovery)
        self.assertIsNotNone(climate_context.competitor_discovery)

        agri_text = " ".join([c.name + " " + c.description for c in agri_analysis.direct_competitors] + agri_analysis.market_leaders).lower()
        fin_text = " ".join([c.name + " " + c.description for c in fin_analysis.direct_competitors] + fin_analysis.market_leaders).lower()
        climate_text = " ".join([c.name + " " + c.description for c in climate_analysis.direct_competitors] + climate_analysis.market_leaders).lower()

        # Domain differentiation check - competitors must be unique and relevant
